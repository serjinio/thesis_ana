...
void draw_cut_marker_lines(double vertR, TH1* hist) {
  double xlim = vertR;
  s13::ana::draw_marker_line(xlim, hist->GetMinimum(),
                             xlim, hist->GetMaximum());
}
...
void draw_angdep_sn(s13::ana::RootScript& script,
  ...
  for (size_t i = 0; i < alg_phi_cut->cut_dists.size(); i++) {
    ...
    for (size_t c = 0; c < alg_phi_cut->cuts_range().size(); c++) {
      ...
      draw_cut_marker_lines(alg_phi_cut->cuts_range()[c].vertexXY_radius,
                            alg_phi_cut->cut_dists[i]);
      auto cut_width_text =
        s13::ana::tstrfmt("VertR < %.2f",
                          alg_phi_cut->cuts_range()[c].vertexXY_radius);
      s13::ana::draw_text(cut_width_text);

      script.cd();
      auto s1 = s13::ana::draw_thstack({alg_phi_cut->sn_tot[i][c],
            alg_phi_cut->sn_out[i][c]},
        "TOT & OUT vertR cut", "NOSTACK");
...
void vertR_ang_dep() {
  ...
  auto compute_cut_dist_evt = [](const s13::ana::ScatteringEvent& evt) -> double {
    return std::sqrt(std::pow(evt.vert_xpos, 2) + std::pow(evt.vert_ypos, 2));
  };
  auto init_cut_hist = [](double min_theta, double max_theta) -> TH1* {
    return s13::ana::init_vertR_bin_hist(min_theta, max_theta, 20);
  };
  auto phi_alg =
  ...
  auto script_fname =
    s13::ana::tstrfmt("%s_%s_vertR_cut_ang_dep", cli_opts.output_file_basename().data(),
